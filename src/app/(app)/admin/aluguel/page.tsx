...
export default async function RentalsAdminPage() {
  ...
  const items = rawItems.map((item) => ({ ...item, description: item.notes || null, quantity: item.totalQuantity, condition: item.condition || "GOOD", createdAt: item.createdAt.toISOString(), updatedAt: item.updatedAt.toISOString() }));
